 ...
 * v1.0.32 — offline rescue: service worker split into a versioned shell cache + a stable
 *           media cache (bird photos no longer wiped on every update); resilient install;
 *           an offline-readiness pill with a "save photos for offline" action.
 */
...
window.APP5=function(UNIC,SMETA,MAPIMG){
 ...
 h+='<div id="appSetup" class="sans" style="margin:14px 0 6px;background:var(--raised);border:1px solid var(--rule);border-radius:10px;padding:13px 16px 14px">'+
   ...
   '<div style="flex:1;min-width:190px"><b>3 · Notes &amp; reference</b><br><span style="color:var(--soft)">Tap any species for its account, photo &amp; names. Tick sightings, keep a per‑day <b>field journal</b> (§8).</span></div>'+
   '<div style="flex:1;min-width:190px"><b>4 · Back it up, every day</b><br><span style="color:var(--soft)">Your notes live <b>on this phone only</b> — nothing is on a server. At the end of each day tap <b>Export notes (JSON)</b> (§8) and keep the file. The app will tell you when there is unsaved work.</span></div>'+
   '</div></div>';
 // 1 South Africa
...
window.__wire5=function(UNIC,SMETA){
 ...
 function paintColHeads(){$$('#matrix .colh').forEach(function(th){var k=th.dataset.site;if(!k)return;var c=siteChipCol(k);th.style.color=c;th.style.borderBottomColor=c;});}
 ...
 function sortRows(){var tb=$('#matrix tbody');if(!tb)return;$$('.grphdr',tb).forEach(function(h){h.remove();});var rws=$$('tr.org',tb);var sm=seenSpeciesMap();
  ...
  // 'my lists': the species you have curated come first — focal, then tour, then anything on any
  // other list, then the rest. Within a tier, A→Z. PR-K made this nearly free: it is just
  // membership. (This is the reshaped "focal/tour/normal sort" backlog item.)
  function listRank(o){if(inList('focal',o.k))return 0;if(inList('tour',o.k))return 1;
   var ids=listIds();for(var i=0;i<ids.length;i++){if(ids[i]!=='focal'&&ids[i]!=='tour'&&inList(ids[i],o.k))return 2;}
   return 3;}
  var base=S.sort==='list'?function(a,b){return (listRank(a)-listRank(b))||nm(a).localeCompare(nm(b));}:S.sort==='za'?function(a,b){return nm(b).localeCompare(nm(a));}:S.sort==='tax'?function(a,b){return ((order[a.g]||0)-(order[b.g]||0))||((a.cl||'').localeCompare(b.cl))||((a.o||'').localeCompare(b.o))||((a.f||'').localeCompare(b.f))||nm(a).localeCompare(nm(b));}:function(a,b){return nm(a).localeCompare(nm(b));};
 ...
 // eBird hotspots for a site: tappable links to the live hotspot page + the eBird species count.
 function ebHot(s){if(!s.hs||!s.hs.length)return '';var links=s.hs.map(function(h){return '<a class="ebhot" href="https://ebird.org/hotspot/'+esc(h[1])+'" target="_blank" rel="noopener" style="color:'+C.acacia+';text-decoration:none">'+esc(h[0])+' ↗</a>';}).join('<br>');return '<div style="margin-top:9px;border-top:1px solid '+C.rule+';padding-top:8px"><div style="font-size:9.5px;font-weight:700;letter-spacing:.4px;color:'+C.acacia+';text-transform:uppercase">eBird hotspots'+(s.ebn?' · '+s.ebn+' species recorded':'')+'</div><div style="font-size:11.5px;line-height:1.6;margin-top:2px">'+links+'</div></div>';}
 ...
 function renderRails(){var L=$('#mapLeft'),R=$('#mapRight');if(!L||!R)return;
  ...
  L.innerHTML='<div style="font-size:10px;font-weight:700;letter-spacing:.7px;color:'+C.terra+';text-transform:uppercase">'+esc(s.region)+' · '+esc(s.date)+'</div><div style="font-family:var(--serif);font-size:17px;font-weight:600;margin:2px 0 3px">'+esc(s.label)+'</div>'+(s.uncertain?'<div style="display:inline-block;background:#fff5ec;border:1px dashed '+C.terra+';color:#8a4a33;border-radius:5px;padding:1px 7px;font-size:10.5px;margin-bottom:5px">⚑ location approximate</div>':'')+'<div style="font-size:11px;color:var(--soft);margin-bottom:8px">'+esc(s.elev||'')+'</div>'+acct('Access',a.ac)+acct('Physiography',a.ph)+acct('Vegetation',a.vg)+acct('Best time',a.bt)+acct('People &amp; history',a.hi)+ebHot(s);
 ...
 $('#themeToggle').onclick=function(){root.dataset.theme=root.dataset.theme==='dark'?'light':'dark';try{localStorage.setItem('sa_theme',root.dataset.theme);}catch(e){}renderMap();paintSiteChips();paintColHeads();};
